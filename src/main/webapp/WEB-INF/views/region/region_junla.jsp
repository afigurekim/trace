...
$(function(){
		if(window.location.href=="http://localhost:8181/region/junla" || window.location.href.includes("http://localhost:8181/region/junla?page="))
		{
			$("#junla").css('background-color','wheat');
		}	
	...
	<div class="text-center">
		<ul class="pagination">
			<c:if test="${pageMaker.prev}">
				<li><a href="/region/junla?page=${pageMaker.startPage-1}">&laquo;</a></li>
			</c:if>
			
			<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
				<li 
					<c:out value="${pageMaker.cri.page == idx?'class=active':''}"/>>
				<a href="/region/junla?page=${idx}">${idx}</a>
				</li>
			</c:forEach>
			
			<c:if test="${pageMaker.next && pageMaker.endPage>0 }">
				<li><a href="/region/junla?page=${pageMaker.endPage +1}">&raquo;</a></li>
			</c:if>
