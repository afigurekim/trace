...
$(function(){
		if(window.location.href=="http://localhost:8181/thema/date" || window.location.href.includes("http://localhost:8181/thema/date?page="))
		{
			$("#date").css('background-color','wheat');
		}	
	...
	<div >
      	...
      	<div class="pl-cate" id="jh">
			...
			<ul class="plc-list" id="sub_category">
				<li><a href="/thema" id="allperiod" style="margin-left: -40px;">전체보기</a></li>
	...
	<div class="text-center">
		<ul class="pagination">
			<c:if test="${pageMaker.prev}">
				<li><a href="/thema/date?page=${pageMaker.startPage-1}">&laquo;</a></li>
			</c:if>
			
			<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
				<li 
					<c:out value="${pageMaker.cri.page == idx?'class=active':''}"/>>
				<a href="/thema/date?page=${idx}">${idx}</a>
				</li>
			</c:forEach>
			
			<c:if test="${pageMaker.next && pageMaker.endPage>0 }">
				<li><a href="/thema/date?page=${pageMaker.endPage +1}">&raquo;</a></li>
			</c:if>
